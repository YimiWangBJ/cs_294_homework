...
Example usage:
    python dagger.py experts/RoboschoolHumanoid-v1.py RoboschoolHumanoid-v1_20_data.pkl \
...
def imitation_learning(obs_data, act_data, sess):
    ...
    for i in range(2): # 2 epochs b/c I have a shit machine
        print('    epoch: '+str(i))
        mini_batch_sz = 256
...
# DAGGER
dagger_passes = 3
new_observations, new_exp_actions = [], []
for dagger_i in range(dagger_passes):
    print('dagger pass: '+str(dagger_i))
    obs_data = np.vstack((obs_data, new_observations)) if len(new_observations) else obs_data
    act_data = np.vstack((act_data, new_exp_actions)) if len(new_exp_actions) else act_data
    ...
    with tf.Session() as sess:
        ...
        imitation_learning(np.array(obs_data), np.array(act_data), sess)
