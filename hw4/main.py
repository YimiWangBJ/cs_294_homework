...
def discount(x, gamma):
    ...
    """
    return scipy.signal.lfilter([1],[1,-gamma],x[::-1], axis=0)[::-1]  # what the flying fuck
...
class NnValueFunction(object):
    ...
    def __init__(self, n_epochs, stepsize):
        self._tf_v = None  # initialize as None so we know network not built
        self._n_epochs = n_epochs
        self._stepsize = stepsize

    def init_network(self, input_shape, output_shape):
        h_units1, h_units2 = 32, 16

        self._tf_X = tf.placeholder(tf.float32, shape=[None, input_shape[1]])
        self._tf_v = tf.placeholder(tf.float32, shape=[None, output_shape[1]])

        self._tf_v_pred = slim.stack(self._tf_X, slim.fully_connected,
                                   [input_shape[1], h_units1, h_units2,
                                    output_shape[1]], activation_fn=lrelu)
        self._tf_v_pred = tf.squeeze(self._tf_v_pred)

        self._tf_loss = (self._tf_v_pred - self._tf_v)**2
        self._optimizer = tf.train.AdamOptimizer(learning_rate=1e-1)
        self._train = self._optimizer.minimize(self._tf_loss)

        self._sess = tf.Session()
        self._sess.run(tf.global_variables_initializer())

    def fit(self, X, y):
        inputs = self.preproc(X)
        y = y if len(y.shape)>1 else np.reshape(y, (-1, 1))
        if self._tf_v is None:
            self.init_network(inputs.shape, y.shape)

        for _ in range(self._n_epochs):
            _ = self._sess.run(self._train,
                                    feed_dict={self._tf_X: inputs,
                                               self._tf_v: y})

    def predict(self, X):
        if self._tf_v is None:
            return np.zeros(X.shape[0])
        return self._sess.run(self._tf_v_pred, feed_dict={self._tf_X: self.preproc(X)})
...
def main_pendulum(logdir, seed, n_iter, gamma, min_timesteps_per_batch,
                  initial_stepsize, desired_kl, vf_type, vf_params,
                  animate=False):
    ...
    sy_h2 = slim.stack(sy_ob_no, slim.fully_connected,
                       [ob_dim, n_hidden_1, n_hidden_2], activation_fn=lrelu)

    # Gaussian params
    sy_mean_na = slim.fully_connected(sy_h2, ac_dim, activation_fn=None)

    sy_logstd_a = tf.get_variable("logstdev", [ac_dim], initializer=tf.zeros_initializer())

    std_a= tf.exp(sy_logstd_a)
    actions_distribution = tf.contrib.distributions.Normal(loc=tf.squeeze(sy_mean_na), scale=std_a)

    sy_sampled_ac = tf.squeeze(sy_mean_na) + tf.random_normal([ac_dim]) * std_a
    sy_logprob_n = actions_distribution.log_prob(sy_ac_n)
    ...
    old_actions_distribution = tf.contrib.distributions.Normal(loc=tf.squeeze(sy_oldmean_na),
                                                           scale=oldstd_a)

    sy_kl = tf.reduce_mean(tf.contrib.distributions.kl_divergence(actions_distribution,
                                                       old_actions_distribution))
    sy_ent = tf.reduce_mean(actions_distribution.entropy())
    ...
    for i in range(n_iter):
        ...
        # Build arrays for policy update
        ob_no = np.concatenate([path["observation"] for path in paths])
        ac_n = np.squeeze(np.concatenate([path["action"] for path in paths]))
        adv_n = np.concatenate(advs)
        standardized_adv_n = (adv_n - adv_n.mean()) / (adv_n.std() + 1e-8)
        vtarg_n = np.concatenate(vtargs)
        vpred_n = np.concatenate(vpreds)
        ...
        # Policy update
        _, oldmean_na, oldlogstd_a = sess.run([update_op, sy_mean_na,
                                               sy_logstd_a],
